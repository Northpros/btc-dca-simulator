...
export default function DCASimulator() {
  ...
  // Computes 60-day vol, then ranks it against the asset's own full historical vol range.
  // Bottom percentile (calm for THIS asset) = aggressive offset, top = cautious.
  const autoRiskOffset = useMemo(() => {
    if (dailyData.length < 120) return -0.05;

    // Compute rolling 60-day vol for the entire history
    const VOL_WINDOW = 60;
    const allVols = [];
    ...
    const percentile = rank / (sorted.length - 1); // 0 = lowest vol ever, 1 = highest

    // Map percentile to offset:
    //   0th percentile (calmest for this asset) -> -0.10 (very aggressive, buy more)
    //   50th percentile (normal)                -> -0.05 (moderate)
    //   100th percentile (most volatile)        -> -0.01 (cautious, wait for deeper dips)
